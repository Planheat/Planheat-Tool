# -*- coding: utf-8 -*-
"""
Created on Wed Jun 13 17:00:00 2018
...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'addUsefulEnergyDlg_Tertiary.ui'), resource_suffix='')

styleInNormal  = "color: green; background-color: #cbffcf "
styleReadOnly  = "color: green; background-color: #ffff7f"

class AddUsefulEnergySourceDlgTertiary(QtWidgets.QDialog, FORM_CLASS):
    ...
    def doConnectActions(self):
      self.pushButtonOK_Tertiary.clicked.connect(self.doParamsConfirmed)
    ...
    def doParamsConfirmed(self):
      ...
      residential_results = []
      file_name = master_mapping_config.CMM_WIZARD_RESULT_FILE_NAME
      if file_name in os.listdir(folder):
        with open(os.path.join(folder, file_name), newline='') as csvfile:
          reader = csv.reader(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
          for row in reader:
            if row and row[0].startswith("residential_"):
              residential_results.append(row)

      print(residential_results)
      ...
      with f as csvfile:
        ...
        for energy in resHeatHot.keys():
          spamwriter.writerow(['tertiary_heating_hot_extracted_MWh_y_' + ""] + [(resHeatHot[energy])])
        ...
        for energy in resCOOLING.keys():
          spamwriter.writerow(['tertiary_heating_cooling_extracted_MWh_y_' + ""] + [(resCOOLING[energy])]) 

        for energy in residential_results:
    ...
    def __init__(self,parent=None):
        super(AddUsefulEnergySourceDlgTertiary, self).__init__(parent)
