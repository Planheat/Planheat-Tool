...
def prioritization_algorithm(weights=(10, 0, 10, 2, 5, 7, 0, 5, 4, 0, 3, 0, 0, 0, 0, 0)):
    # print("Prioritization_algorithm.py, prioritization_algorithm(), weights:", weights)
    if not len(weights) == vito_criteria:
        print("prioritization_algorithm: input weights not conform: ", weights)
        return None
...
def single_table_calculation(data, start_cell, weight, debug):
    ...
    for i in range(tb_rows):
        output.append(0)
        if debug:
            # print(data.values[start_cell[0] + i][start_cell[1]:start_cell[1] + tb_columns])
            pass
